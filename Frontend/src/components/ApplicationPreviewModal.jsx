...
import { AlertCircle, CheckCircle2, LoaderCircle, X, Zap } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-zinc-950/70 p-4" role="dialog" aria-modal="true">
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto border border-zinc-700 bg-white p-6 text-zinc-900 shadow-2xl dark:bg-zinc-900 dark:text-zinc-100">
        ...
        {data && <div className="space-y-6">
          <div className="grid gap-3 sm:grid-cols-3"><div className="border border-zinc-200 p-4 dark:border-zinc-800"><span className="text-[10px] font-mono uppercase text-zinc-500">Fit score</span><strong className="mt-1 block text-3xl font-black">{analysis.score}%</strong></div><div className="border border-zinc-200 p-4 dark:border-zinc-800"><span className="text-[10px] font-mono uppercase text-zinc-500">Eligibility</span><strong className={`mt-2 block text-sm uppercase ${analysis.eligible ? 'text-emerald-600' : 'text-amber-600'}`}>{analysis.eligible ? 'Meets requirements' : 'Review skill gaps'}</strong></div><div className="border border-zinc-200 p-4 dark:border-zinc-800"><span className="text-[10px] font-mono uppercase text-zinc-500">Analysis</span><span className="mt-2 flex items-center gap-1 text-sm"><Zap size={14} /> {analysis.provider}</span></div></div>
